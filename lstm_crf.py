...
from config import Reader, Config, lr_decay, simple_batching, evaluate_batch_insts, get_optimizer, write_results, batching_list_instances
import time
...
def train_model(config: Config, train_insts: List[List[Instance]], dev_insts: List[Instance], test_insts: List[Instance]):
    ...
    print("[Training Info] number of training instances: %d" % (len(train_insts)))
    ...
    model_name = model_folder + '/best.pt' #model names for each fold
    train_batches = batching_list_instances(config, train_insts)
    train_one(config=config, train_batches = train_batches,
